...
Copy the [apache/mailcow.conf](apache/mailcow.conf) and the [apache/mailman.conf](apache/mailman.conf) to the *Apache* conf folder `sites-available` (e.g. under `/etc/apache2/sites-available`).
